...
open class JsonConfigurationCollection<E>: ConfigurationCollection<E> {
    ...
    override fun find(key: String, value: Any): Collection<E> {
        val objs = mutableListOf<E>()
    ...
    fun debug() {
        configuration.forEach { it ->
            it!!::class.java.declaredFields.forEach { println(it.name) }
        }
    }
}